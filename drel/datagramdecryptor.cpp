...
namespace DragonSRP
{
	...
	void DatagramDecryptor::decryptAndVerifyMac(unsigned char *in, unsigned int inLen, unsigned char *data, unsigned int *dataLen, uint64_t *seqNum)
	{			
		...
		memcpy(dataLen, in, DSRP_ENCPARAM_LEN_SIZE); // May cause endianess issues
		if (realDataLen != *dataLen) throw DsrpException("Malformed packet data field length. Possible attack detected.");
